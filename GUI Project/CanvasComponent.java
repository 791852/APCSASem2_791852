...
public class CanvasComponent extends JComponent implements MouseListener, MouseMotionListener, ActionListener, KeyListener
{
    ...
    public boolean shapeNotSelected;
    ...
    public void mousePressed(MouseEvent e){
        ...
        if(mouseFromX >=gutterX || mouseFromX > xposition && mouseFromX <= xposition+gutterX  && mouseFromY >= gutterY|| mouseFromY >yposition && mouseFromY <= yposition+gutterY){
            shapeNotSelected = true; //sets this to true if mouse outside outside box and outside gutters
        }
    }
    ...
    public void actionPerformed(ActionEvent e){ 
        ...
        if(shapeNotSelected && mouseFromX != xposition && mouseFromY != yposition){ //checking if mouse clicked outside the shape
            animationDeltaX = (mouseFromX-xposition/2)/10;
            animationDeltaY = (mouseFromY-yposition/2)/10;
            xposition = mouseFromX + animationDeltaX*motionSpeed; //sets new position of shape to where the mouse is
            yposition = mouseFromY + animationDeltaY*motionSpeed;
        }
